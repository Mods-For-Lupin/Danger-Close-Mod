...
import io.github.jason13official.danger_close.impl.common.compat.SoulFireD;
...
public class DangerClose {
  ...
  public static boolean SOUL_FIRE_D_FOUND = false;

  public static void init() {

    Sailing.register(Constants.MOD_ID, MonoLib.createFilename(Constants.MOD_ID, "1.20.1", "1.0.0"));

    ModConfigIO.getOrCreate();

    SOUL_FIRE_D_FOUND = Services.PLATFORM.isModLoaded("soul_fire_d");
  }
  ...
  public static void detect(ServerLevel level, LivingEntity living) {
    ...
    if (living.onGround()) {
      ...
      boolean hasFrostWalker = EnchantmentHelper.getEnchantmentLevel(Enchantments.FROST_WALKER, living) > 0;
  ...
  private static void detectTorches(LivingEntity living, List<TagKey<Block>> insideReStreamable, List<TagKey<Block>> belowReStreamable, boolean hasFrostWalker) {
    ...
    if (ServerConfig.TORCHES_BURN.get() && !hasFrostWalker && (inNormal || onNormal)) {
      ...
      if (SOUL_FIRE_D_FOUND) {
        SoulFireD.immolateSoul(living, 2);
        return;
      }
